...
所有变量（包括基本类型和引用类型）都存在于一个执行环境（也称作用域）中，这个执行环境决定了变量的生命周期，以及哪一部分代码可以访问其中的变量。
这个文档目录：
  - 执行环境及作用域  
  - 延长作用域链  
  - 没有块级作用域  
...
4. 函数的局部环境可以访问函数作用域中的变量，也可以访问父环境中的变量，也可以访问全局环境中的变量。
5. 全局环境只能访问全局环境中定义的变量和函数，不能访问子环境中的变量和函数。  
